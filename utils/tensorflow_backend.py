...
#    idx_not_minus_1 = tf.where(tf.not_equal(target, tf.constant(-1, dtype=tf.float32)))
#    r,c = target.get_shape().as_list()
#    unc = tf.constant(0, shape=[r,c])
#    delta = tf.SpareTensor(idx_not_minus_1, [1], [r,c])
#    unc = unc + tf.sparse_tensor_to_dense(delta)
#    entropy = tf.reduce_mean(tf.losses.compute_weighted_loss(weights=tf.cast(unc, tf.float32)))
#    return tf.nn.sigmoid_cross_entropy_with_logits(labels=target,
#                                                   logits=output, entropy)
